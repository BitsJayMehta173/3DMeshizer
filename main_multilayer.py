...
def render_mesh_comparison(orig_mesh_path, recon_mesh, save_path):
    ...
    axes[1].set_title("Reconstructed (Multi-Layer Depth Peeling)")
    axes[1].axis('off')
...
def run_multilayer(num_layers=4, resolution=256):
    print(f"--- Running Multi-Layer Depth Peeling (Layers: {num_layers}) ---")
    ...
    # Encode
    encoded_data = encode_multilayer(mesh, resolution=resolution, num_layers=num_layers)
    
    print("Compressing all layer maps to calculate total file size...")
    total_bytes, _ = compress_png(encoded_data['maps'], "data/compressed/multilayer_png")
    kb_size = total_bytes / 1024.0
    print(f"Total Compressed Size (PNG): {kb_size:.2f} KB for {num_layers * 6} maps.")
    
    # Decode
    pcd = decode_multilayer(encoded_data, resolution=resolution)
    ...
    render_mesh_comparison(mesh_path, recon_mesh, "results/plots/multilayer_comparison.png")
    
    # Save the mesh for interactive viewing
    os.makedirs("data/decoded", exist_ok=True)
    o3d.io.write_triangle_mesh("data/decoded/multilayer_reconstructed.ply", recon_mesh)
    print("Saved reconstructed mesh to data/decoded/multilayer_reconstructed.ply")
    
    return recon_mesh, metrics

if __name__ == "__main__":
    run_multilayer(num_layers=4, resolution=256)
